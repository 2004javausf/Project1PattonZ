...
/**
 * Servlet implementation class SupervisorLoginServlet
 */
public class SupervisorLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Supervisor c = SupervisorLoginServlet.slog(userN, passW);
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("/supervisorlogin.html");
			rd.include(request, response);
		}
	}
	public static Supervisor slog (String unInput, String passInput) {
		LoginDAOImpl ldi = new LoginDAOImpl();
		try {
			for (int i = 0; i < ldi.getSupervisorList().size(); i++) {
				String user_name = ldi.getSupervisorList().get(i).getUser_name();
				String pass_word = ldi.getSupervisorList().get(i).getPass_word();
				if(unInput.equals(user_name) && passInput.equals(pass_word)) {
					return ldi.getSupervisorList().get(i);
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
